...
func (b *SSH) StartProcess(cmd string, args ...string) (Waiter, io.Writer, io.Reader, io.Reader, error) {
	...
	if err != nil {
		return nil, nil, nil, nil, errors.New(err.Error() + ". Could not get hold of the SSH session's stdin stream")
	}

	stdout, err := b.Session.StdoutPipe()
	if err != nil {
		return nil, nil, nil, nil, errors.New(err.Error() + ".Could not get hold of the SSH session's stdout stream")
	}

	stderr, err := b.Session.StderrPipe()
	if err != nil {
		return nil, nil, nil, nil, errors.New(err.Error() + ". Could not get hold of the SSH session's stderr stream")
	}

	err = b.Session.Start(b.createCmd(cmd, args))
	if err != nil {
		return nil, nil, nil, nil, errors.New(err.Error() + ". Could not spawn process via SSH")
	}
